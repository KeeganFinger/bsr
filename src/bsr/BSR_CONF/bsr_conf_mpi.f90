...
! ... target orbitals:
      ...
      Call br_orb
      if(nwf.ne.nwt) then
        if(myid.eq.0) write(pri, '(a)') 'nwf in target.bsw <> nwt'
      ...
      if(myid.eq.0) write(pri,'(/a,T33,i8/)') &
       'number of target configurations:',ncfg-ncfg_phys

      if(ncfg-ncfg_phys.ne.nct) then
        if(myid.eq.0) write(pri, '(a)') 'ncfg_target <> nct from target'
        Call MPI_ABORT(MPI_COMM_WORLD,1,ierr)
      endif
...
! ... read partial waves:
      ...
      if(nlsp.le.0) then
        if(myid.eq.0) write(pri, '(a)') 'bsr_conf: nlsp <= 0 ?'
        Call MPI_ABORT(MPI_COMM_WORLD,1,ierr)
      endif
...
! ... loop other partial waves:
      ...
      Do ilsp=1,nlsp
        Call MPI_BARRIER(MPI_COMM_WORLD,ierr)
        if(myid.ne.modulo(ilsp,nprocs)) Cycle
...
! ... define channels configurations:
        ...
        Select Case (COUPLING)
          ...
          Case('JJ');  Call SUB_JJ
          Case default
            if(myid.eq.0) write(pri, '(a)') 'unknown coupling, possible: LS,JK,JJ'
            Call MPI_ABORT(MPI_COMM_WORLD,1,ierr)
        End Select
...
!======================================================================
        ...
        ncfg=ncfg_targ; lcfg=lcfg_targ; nwf=nwf_pert; ic_targ=jc_targ
...
! ... define scattering channels with phys.target states:
        ...
        Select Case (COUPLING)
          ...
          Case('JJ');  Call SUB_JJ
        End Select
        if(nch.ne.nch_save) then
          if(myid.eq.0) write(pri, '(a)') 'nch <> nch_save'
          Call MPI_ABORT(MPI_COMM_WORLD,1,ierr)
        endif
