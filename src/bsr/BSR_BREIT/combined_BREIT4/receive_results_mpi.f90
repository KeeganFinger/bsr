...
! ... Receive det info
      ...
      Allocate(NPD(ldet))
      Call MPI_RECV(NPD,ldet,MPI_INTEGER,MPI_ANY_SOURCE,17,&
                    MPI_COMM_WORLD,status,ierr)

! ... Receive def info
      Call MPI_RECV(ndef,1,MPI_INTEGER,MPI_ANY_SOURCE,18,&
                    MPI_COMM_WORLD,status,ierr)
      Call MPI_RECV(ldef,1,MPI_INTEGER,MPI_ANY_SOURCE,19,&
                    MPI_COMM_WORLD,status,ierr)
      if(allocated(KPF)) Deallocate(KPF)
      Allocate(KPF(ndef))
      Call MPI_RECV(KPF,ndef,MPI_INTEGER,MPI_ANY_SOURCE,20,&
                    MPI_COMM_WORLD,status,ierr)
      if(allocated(IPF)) Deallocate(IPF)
      Allocate(IPF(ndef))
      Call MPI_RECV(IPF,ndef,MPI_INTEGER,MPI_ANY_SOURCE,21,&
                    MPI_COMM_WORLD,status,ierr)
      if(allocated(NPF)) Deallocate(NPF)
      Allocate(NPF(ldef))
      Call MPI_RECV(NPF,ldef,MPI_INTEGER,MPI_ANY_SOURCE,22,&
                    MPI_COMM_WORLD,status,ierr)
